...
readDerivedDataFiles = function(processedFiles,procol,path){
	...
	for(procFile in processedFiles){
	  ...
	  procData_colnames <- try(fread(file.path(path, procFile), sep = "\t",
	                        header = T, # interpret header
	                        nrows = 0, # header located in line 1
	                        stringsAsFactors = FALSE,
	                        na.strings = c('NA','NULL','null'),
	                        fill = T,
	                        data.table = F));
	  procData_colnames = c("rownames",colnames(procData_colnames))
	  procData_col_ind = c(1, which(procData_colnames %in% procol))
	  procData_colnames = procData_colnames[procData_col_ind]
		procData <- try(fread(file.path(path, procFile), sep = "\t",
						header = FALSE, # interpret header,
						skip = 1,
						select = procData_col_ind, # read only necessary columns
						stringsAsFactors = FALSE,
						na.strings = c('NA','NULL','null'),
						data.table = F)); # return data.frame
		colnames(procData) = procData_colnames
		rownames(procData) = procData$rownames
		procData$rownames = NULL

		procsel = matrix(as.numeric(as.matrix(procData)), nrow = nrow(procData))
